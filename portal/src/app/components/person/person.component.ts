...
export class PersonComponent implements OnInit {
    ...
    buildJson(data) {
        if (data != null) {
            ...
            data = Object.keys(data).map(function (key, index) {
                let obj = {
                    'name': key,
                    'label': self.validateLabel(key),
                    'value': self.validateValue(data[key], key),
                    'order': index
                };
                index++;
                return obj;
    ...
    validateLabel(key) {
        if (key.startsWith('PDF_'))
            return this.adjustString(key.split('PDF_')[1]);
        else if (key.startsWith('IMG_'))
            return this.adjustString(key.split('IMG_')[1]);
        else
            return this.adjustString(key)
    }
